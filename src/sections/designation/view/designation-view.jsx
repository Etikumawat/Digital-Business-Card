...
const headCells = [
  ...
  { id: 'name', numeric: false, disablePadding: false, label: 'Designation' },
  { id: 'department.name', numeric: false, disablePadding: false, label: 'Department' },
...
export default function EnhancedTable() {
  ...
  const fetchUsers = useCallback(
    debounce((search) => {
      ...
      setLoading(true);
      axiosInstance
        .get(apiURL + '/designations', { params })
        .then((res) => {
          setLoading(false);
  ...
  const handleUpdateDialogOpen = (department) => {
    ...
    setDepartmentValue(department.department ? department.department.id : '');
  ...
  const handleCreateDepartment = () => {
    ...
    if (!departmentName) {
      toast.error('Designation name required');
      setLoading(false);
      return;
    }
  ...
  const handleUpdateDepartment = () => {
    ...
    axiosInstance
      .put(`${apiURL}/designations/${selectedDepartment.id}`, data)
      .then((res) => {
        setLoading(false);
        toast.success(res.data.message);
        fetchUsers();
  ...
  const handleDeleteDepartment = () => {
    setLoading(true);
    axiosInstance
      .delete(`${apiURL}/designations/${selectedDepartment.id}`)
      .then((res) => {
        setLoading(false);
        toast.success(res.data.message);
        fetchUsers();
    ...
    <Box sx={{ width: '100%' }}>
                      ...
                      <>
                        <>
                          ...
                          >
                            ...
                            <TableCell>{row.department ? row.department.name : ''}</TableCell>
      ...
      <Dialog open={deleteDialogOpen} onClose={handleDeleteDialogClose}>
        ...
        <DialogContent>
          <Typography>
            {' '}
            Do you want to delete this Designation "{selectedDepartment?.name}"?
          </Typography>
        </DialogContent>
        <DialogActions>
